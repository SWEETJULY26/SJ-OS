# Excalidraw — Full mode

Detailed, structured diagrams: architecture, nested containers, multi-zone systems, anything that needs planning and a feedback loop.

The JSON wrapper, save location (`diagrams/[concept-slug].excalidraw`), and how-to-open delivery instructions live in the router `SKILL.md` — follow those at Step 5.
...
1. Save to `diagrams/[concept-slug].excalidraw` (always use the `diagrams/` subfolder to keep the project root clean)
...
4. Tell the user how to use the file (see the delivery block in `SKILL.md`)
