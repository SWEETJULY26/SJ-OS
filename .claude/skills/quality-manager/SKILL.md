...
**SOP catalog as canonical reference — Quality's own SOPs only.** The catalog at `references/sop-catalog.md` is the runtime index of which Quality SOP revision (SKN-OPS-001–007) is current. Sub-skills query the catalog before significant writes. The procedure text itself is canonical in `sops/` (markdown, not SharePoint or Supabase, per the 2026-08-04 decision); the catalog tracks revision, effective date, ratification status, owner role, and in-flight drafts on top of that. As of 2026-08-06, Regulatory, PD, and Ops each keep this same pattern in their own catalog — `regulatory-manager/references/sop-catalog.md`, `asana-pd-manager/references/sop-catalog.md`, `sjs-ops-system/references/sop-catalog.md`. quality-manager still owns the shared process all four follow (ratification protocol, annual review protocol, numbering policy) at `references/sop-program.md` — that file, not this one, is the cross-function source of truth for how a SOP gets ratified or reviewed, and for the one shared SKN-OPS-NNN numbering sequence.
