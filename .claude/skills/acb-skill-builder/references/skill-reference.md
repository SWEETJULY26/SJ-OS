...
Technical reference for Claude Code skills: every frontmatter field, advanced patterns, argument passing, troubleshooting, and how skills fit into a project. Read the section you need — you don't have to load the whole file.

Source: https://code.claude.com/docs/en/skills

## Contents

- **Understanding CLAUDE.md vs Skills** — where an instruction belongs, and how the two layer
- **Frontmatter Field Reference** — every field, with defaults
- **Invocation Control Matrix** — how `disable-model-invocation` and `user-invocable` interact
- **String Substitutions** — `$ARGUMENTS`, `$N`, `${CLAUDE_SESSION_ID}`, and the auto-append fallback
- **Skill File Locations** — personal vs project vs plugin, and priority order
- **`allowed-tools` Syntax Guide** — comma lists and `Bash(git *)`-style globs
- **Advanced Patterns** — dynamic context injection (`!`command``), `context: fork` subagents, hooks, bundled visual scripts, extended thinking
- **Permissions and Access Control** — restricting which skills Claude can invoke
- **Troubleshooting** — skill won't trigger, triggers too often, not all skills visible, fork returns nothing, args not substituting
