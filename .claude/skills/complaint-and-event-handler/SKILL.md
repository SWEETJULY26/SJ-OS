...
- *Action:* Pull tasks from the SJ Skin Complaint Log. Group by lens. Compare to baseline. Surface anomalies. If a threshold breach shows up (single batch with 3+ complaints in 30 days, single SKU with 5+ same-type complaints in 30 days), draft a CAPA brief and stage it for handoff to capa-coordinator (see Job 6).
- *HITL:* none — read-only.
