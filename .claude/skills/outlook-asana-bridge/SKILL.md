...
| `ops` | PO number, PO ack, ship date, freight, carrier, customs, duty, 3PL, OC3PL, Logiwa, inventory, low stock, OOS (out-of-stock — not the lab signal), reorder, S&OP, demand, forecast, fulfillment, late shipment, RTS, replacement, Shopify order, **CDW mini-box / shipping-packaging outreach — CDW is a PD supplier for carton/packaging dev, but mini-box shipping-packaging correspondence is a shipping-packaging concern, not product development; route to `ops` even though CDW would otherwise match the `pd` supplier-domain signal above** |
