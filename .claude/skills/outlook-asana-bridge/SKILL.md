...
| `ops` | PO number, PO ack, ship date, freight, carrier, customs, duty, 3PL, OC3PL, Logiwa, **Logiwa Receipt Order Report — senders `noreply@wmsnotification.com` and `noreply@wmssystem.logiwa.com`, subject `Receipt Order Report for RO#… of Sweet July`; flag `⚙️ PLM Bridge` and route to `inventory-manager` Job 2 per `inventory-manager/references/logiwa-receipt-report.md`, since the payload is a PLM receipt write**, inventory, low stock, OOS (out-of-stock — not the lab signal), reorder, S&OP, demand, forecast, fulfillment, late shipment, RTS, replacement, Shopify order, **CDW mini-box / shipping-packaging outreach — CDW is a PD supplier for carton/packaging dev, but mini-box shipping-packaging correspondence is a shipping-packaging concern, not product development; route to `ops` even though CDW would otherwise match the `pd` supplier-domain signal above** |
