...
- *Trigger:* **a Logiwa Receipt Order Report email** (the primary signal — see `references/logiwa-receipt-report.md`), outlook-plm-bridge logs goods received notification, the operator marks a PO received, "log the receipt for PO #X", "PO #X arrived", "scan for receipt reports"
- *Primary source:* Logiwa emails a `Receipt Order Report for RO#<id> of Sweet July` per receipt order, from `noreply@wmsnotification.com` or `noreply@wmssystem.logiwa.com`, with the line detail as an inline table in the body. `references/logiwa-receipt-report.md` carries the column spec, the three RO classes (PO-linked / returns / named movement), the PO-matching rule, and the reconciliation logic. Read it before parsing one. Two things it warns about that will bite otherwise: the report's `PO DIFFERENCE = 0` means that receipt order was fully received and says nothing about whether the PLM PO is complete, and the report has **no batch column**, so batches always come from the operator.
