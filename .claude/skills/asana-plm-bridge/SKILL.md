...
> ⚠️ **Known bug, found 2026-07-29 — `plm/asana` is not an accepted value.** The live check constraint `wiki_pages_last_source_check` allows only `email/outlook`, `meeting/fireflies`, `plm/outlook`, `manual`, `seed`. Every write documented in this skill with `last_source = 'plm/asana'` (five places, including Flow 5 and the Job 0 `UPDATE` below) will fail with a 23514 constraint violation. The taxonomy clearly intends this value — `plm/outlook` is its sibling — so the fix is a one-line migration adding `plm/asana` to the constraint, which needs Alvin's go-ahead. **Until that lands, write `manual` and put the real source in `last_source_ref`** (this skill already sets `last_source_ref` to the Asana task GID, which is what actually distinguishes the write). Don't silently skip the wiki write.
