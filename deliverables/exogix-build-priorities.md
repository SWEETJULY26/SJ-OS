...
| Skill | What it does | Trigger today | Governing SOPs |
|---|---|---|---|
...
| `plm-assistant` | Sole writer to PLM (Supabase). Everything above writes through it | Called by other skills | All PLM writes go through one layer |

**Hard requirement in this phase:** nothing vendor-facing sends without my approval. Today that gate lives in Asana statuses; keep an equivalent gate in whatever you propose.

## Phase 2 — Inventory, daily order ops, logistics

The rest of the daily execution layer. Same shape as Phase 1: scheduled, repetitive parsing and state movement.
...
Fully proceduralized (SKN-OPS-001 through 009) and the highest-stakes domain in the system. This is where Ibrahim's point about probabilistic errors and FDA liability lands hardest. Every phase gate here keeps a human approval: nothing files to an agency or responds to a customer without sign-off.
...
Recurring render jobs currently burning full Claude sessions on a schedule. The layout and data pulls never change; only the narrative summary needs judgment.
...
3. The queue registry and SOP set are maintained in the repo and will keep evolving. Treat the repo as the spec source, and we should agree on a change-notice convention so process changes flow into the new system, which was the adaptability point from my vision on the call.
