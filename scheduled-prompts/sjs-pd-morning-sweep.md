# Scheduled task: sjs-pd-morning-sweep
...
Execute fully. Do not ask permission. Do not produce a written brief at the chat level.
...
ALWAYS post a recap comment on Asana task GID 1214208955674591 at the end of every run, even when nothing fires. Match the HTML pattern from the most recent morning sweep comments:

- Header line: "🌅 sjs-pd-morning-sweep, YYYY-MM-DD HH:MM PT"
- Window line + dedup note vs prior runs today
- Per-skill sections (Skill 2 Fireflies, Skill 3 Outlook to Asana, Skill 6 Outlook to PLM, Skill 1 + Skill 4) — list sync-backs as `<strong>Task Name</strong>, <a href="URL">GID</a>, note` inside `<ul><li>`; if nothing fired, say so explicitly
- URGENT check block (each criterion checked with result)
- Cross-system flags noted but not actioned (Quality, Regulatory, Margin, Ops, Intel, Founder, Finance)
- Inbox / calendar / internal noise summary
- Next-run dedup keys for the next sweep to skip

REPORTING RULE: Any time the sweep produces a list of back-synced tasks (urgent log entries, recap entries, ad-hoc summary in chat), every task reference MUST include the Asana URL in the format `https://app.asana.com/0/0/{task_gid}/f`. Format as `**Task Name** — [GID](URL) — note` (or HTML equivalent inside the recap). Plain GID-only references are not acceptable.
