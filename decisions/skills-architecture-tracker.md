...
**System-wide note (not yet acted on):** Alvin flagged that Shopify being connected is relevant "for all skills," not just this one. Logged here as a capability to revisit — which other skills would benefit from a direct Shopify read (inventory-manager, oc3pl-order-manager, and sjs-status-reporter are the most likely candidates) is an open question, not something to wire in blind across the suite.

**Not yet done:** re-run the three complaint-and-event-handler evals against the fixed skill to confirm the batch-verification and wiki-context fixes hold, if Alvin wants confirmation.
