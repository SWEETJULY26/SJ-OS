...
**New backlog item — evals:** Zero of the 58 skills have run coverage (with-skill vs. baseline benchmark results), even though `acb-skill-builder` ships a full working eval harness (with-skill vs. baseline subagent runs, quantitative assertions, benchmark viewer — see its `references/running-evals.md`). This is real tooling sitting unused, not a build-from-scratch problem. Scope TBD — 58 skills is a lot to eval at once; likely start with the highest-traffic ones (asana-pd-manager, the four bridges, capa-coordinator, complaint-and-event-handler) rather than all at once.

**Correction found 2026-07-21:** the "zero eval coverage" claim above wasn't quite right — `capa-coordinator/evals/evals.json` and `complaint-and-event-handler/evals/evals.json` already existed (3 well-written test cases each, dated to the original 2026-07-19 repo-consolidation commit, predating this tracker entry). They'd just never been run — no workspace, no grading, no benchmark. Lesson matches the MEMORY.md standing one: verify against the actual file state, not a summary claim, before treating something as a from-scratch gap.
...
Re-run against the fixed skills confirmed the corrections hold (iteration-2, same session).

## Phase 5b — Eval coverage for the remaining four shortlisted skills (2026-07-21)

Alvin deprioritized the `sjs-retail-intel` promo-calendar gap (out of scope, not his to own — pushed to the back of the backlog, see the entry above) and asked to continue eval coverage for the other four originally shortlisted skills: `asana-pd-manager`, `fireflies-asana-bridge`, `capa-coordinator`, `complaint-and-event-handler`. Ran 10 test cases total (2 each for the first two, 3 each for the latter two — capa-coordinator and complaint-and-event-handler already had well-written unused `evals.json` files, kept as-is rather than rewritten) — 20 with-skill/baseline pairs.

**Results:** with-skill passed 100% of assertions across all 10 evals (54/54). Baseline ranged 17%-67%, best on evals closest to general-knowledge-inferable (capa-coordinator's full-lifecycle walkthrough, where domain reasoning about CAPA/RCA carries most of the way) and worst on evals requiring the skill's specific procedural mechanics or live-data verification.

**Real findings surfaced, independent of the eval framing:**
- `asana-pd-manager`'s `references/stage-gate-procedure.md` documents a Formula Tracker section called "Signed Approvals" that doesn't exist anywhere in the live Asana workspace — the actual section is "Approved." Both the with-skill and baseline runs independently confirmed this via live reads. **Fixed 2026-07-21** — Alvin clarified "Signed Approvals" is a custom-field-driven view, not a section: the task moves to the **Approved** section whenever the review/approval field is set to Approved, from whichever section it's currently in (not just from "In Review"). Updated `stage-gate-procedure.md` and `SKILL.md` accordingly. Alvin also flagged a real gap while reviewing this: the IL Review Gate had no dedup check before creating a new `[IL Review Pending — claims-il-and-label-keeper]` task — a re-approval could create a duplicate regulatory task. Added an explicit dedup-check-first step to the gate's trigger rule.
- `capa-coordinator`'s baseline (not the skill) invented a severity value of "Moderate" that doesn't exist in SKN-OPS-005's Critical/Major/Minor band system — a concrete example of the schema-drift risk the skill exists to prevent.
- `complaint-and-event-handler`'s with-skill run caught that "lot 24-11," the batch number in the recall-kickoff eval prompt, doesn't match any real Soursop Vit C batch in PLM or the Complaint Log — it correctly refused to scope a recall against an unverified batch number. Baseline accepted the number at face value without checking.
- Across `capa-coordinator` and `complaint-and-event-handler`, baseline made exactly 3 tool calls on every single run (regardless of task) — it never queried live Asana/PLM/wiki context at all, reasoning purely from the prompt text and general knowledge. That's a structural blind spot specific to skills whose job is intake against live records, not something that improves with a smarter model alone.

**Not yet done:** re-run the asana-pd-manager fig-stage-move eval against the fix above, if Alvin wants confirmation it holds. All originally-shortlisted skills (the four bridges + asana-pd-manager + capa-coordinator + complaint-and-event-handler) now have real eval coverage; 51 of 58 skills in the suite remain unevaluated, out of scope unless Alvin asks to expand further.

**Update — fireflies-asana-bridge cost rescope (2026-07-21, same session):** Alvin reviewed the fireflies-asana-bridge viewer and flagged the token cost as unacceptable for a skill meant to run daily — 172K tokens on the weekly-catchup eval, 130K on a single-item lookup. Root cause wasn't tool-call volume (9-14 calls, cheap); it was full-transcript fetches for every meeting in the window plus an exhaustive report format that restated content and rendered empty sections. Rescoped (now v1.1):
- Default fetch switched from `fireflies_fetch` (full transcript) to `fireflies_get_summary` for every meeting; full-transcript fetch reserved for genuinely ambiguous items or ones hitting an urgency/quality/regulatory signal where precision matters.
- Default catch-up window now sizes to the actual ask ("today's meetings" → today; explicit "this week" → 7 days) instead of a flat 7-day sweep on every run.
- Action Item Report rewritten: dropped the redundant meeting-by-meeting recap, omits empty sections instead of explaining them, caps each item to one line.

**Re-run confirmed (2026-07-21, same session):** both evals re-run against the rescoped skill (iteration-2). Token cost dropped substantially — weekly catch-up 172K → 94K (~45% down), Sorrel lookup 130K → 81K (~38% down). Fetch discipline held: eval-0 pulled summaries for 9 of 10 meetings and only fell back to a full transcript for the one that genuinely needed it (a regulatory signal too thin on specifics in the summary); eval-1 made zero full-transcript fetches, resolving entirely off `fireflies_search`/`fireflies_get_summary`. Report format held to the new one-line-per-item, no-recap, empty-sections-omitted structure. Correctness held — both runs still correctly classified and stopped at the confirmation gate with no writes.

**Update — iteration-2 re-run (2026-07-20, same session):** re-ran the with-skill side of all four evals against the fixed skills. All four v1.2 fixes confirmed against live data: NetSuite sweep not attempted; the HCT tube-proof thread was read to the end and correctly showed proofs already signed/returned (a real behavior change vs. iteration-1, which had flagged them as still pending); the new Flow A PO recognition rule found a genuine receipt-confirmation on PO #100336 buried in a shared punch-list email that the old literal-number search had missed entirely (a `status: Sent → Acknowledged` update now stages correctly).

The OC3PL/Pedrero fix surfaced a sharper root cause than what got written into v1.2: two independent runs found that `outlook_email_search`'s `folderName` parameter reliably returns `NOT_FOUND` on both folders — not intermittently, and retrying the same call doesn't help. Both are real, populated, non-nested Inbox subfolders; the actual working fallback is listing folders via `read_resource` on `mail:///folders/` to get the folder ID and reading directly by ID, or a sender-domain backstop search. Rewrote the guidance in both bridges (now v1.3) to specify that fallback instead of just "retry the exact name."

---

## Phase 5c — capa-coordinator and complaint-and-event-handler viewer feedback (2026-07-22)

**capa-coordinator:** Alvin flagged that most SOPs written before 2026 are likely outdated and need a review pass — this came up looking at the complaint-trend-handoff eval, which walks SKN-OPS-001/SKN-OPS-005 verbatim. **Not acted on yet** — this needs Alvin (or whoever owns SOP ratification, per quality-manager's role) to say which SOPs and what's actually stale before anything gets rewritten; capa-coordinator's own design principle is to surface SOP gaps rather than patch them unilaterally, and a blanket "SOPs are outdated" note doesn't have enough specificity to act on without guessing. Flagging here so it doesn't get lost — worth a dedicated SOP-review pass, likely paired with quality-manager's ratification queue.

**complaint-and-event-handler — three fixes landed (now v5.2, 2026-07-22):**
- **Batch verification at intake (Job 1):** the skill previously just noted a complaint's Product Batch Code without resolving it against anything. Now cross-references it against PLM (via plm-assistant) and, when an order number is present, against OC3PL/Logiwa (via oc3pl-order-manager) to confirm which batch actually shipped — catching customer-misread lot numbers before they propagate into trend analysis (Job 3) or recall scoping (Job 5).
- **Wiki-context inconsistency:** an eval spot-check found the "read the SKU wiki page before live queries" step (documented at the bottom of the SKILL.md) was followed on the SAE eval but not on the intake or recall evals. Moved an explicit reminder to the top of "The six core jobs" so it's a stated prerequisite for every job, not something that depends on which job someone happens to remember to apply it to.
- **Shopify now a direct read source:** Alvin noted Shopify is now connected as a live MCP (it's been in `connections.md` since onboarding, but wasn't wired into this skill's own "Reads via" list) — added it as a source for order/channel verification, complementing oc3pl-order-manager's view.

**System-wide note (resolved 2026-07-22, next session):** the "which other skills" question got picked back up. Checked all three candidates against their actual jobs: oc3pl-order-manager ruled out (Logiwa is already authoritative for its shipment-KPI work; adding Shopify would just duplicate inventory-manager's position tracking). sjs-status-reporter ruled out (PD-only aggregator; inherits any inventory-side Shopify data downstream through inventory-manager's Job 8 handoff rather than needing its own read). Two skills got wired:

- **inventory-manager:** Job 1 (position keeping) and Job 4 (three-way reconciliation) previously named Shopify conceptually but had no defined live source for it — added a direct `mcp__Shopify__get-inventory-levels` read for both, and added Shopify to the "Reads via" integration list.
- **supply-demand-planner (S&OP):** sharper gap than inventory-manager's — Step 1 of the monthly run ("Pull actuals") named Shopify as the DTC source, but the only concretely wired mechanism was manual CSV upload. Wired `mcp__Shopify__run-analytics-query`/`list-orders` as the primary path; CSV drops to fallback-only, flagged in the run summary whenever it fires.

Both logged in full at `decisions/log.md` (2026-07-22 entry). That entry also raised a bigger, unresolved architecture question Alvin surfaced while scoping this: PLM already syncs to Shopify via a separate API integration that feeds the landing hub's inventory dashboard — now that the MCP connector exists, does that API sync still make sense, or should some of that data path move to direct MCP reads? Explicitly not decided — bigger build, needs its own scoping pass, logged as open in the decision entry and flagged inline in both skills' SKILL.md so the two Shopify paths (PLM's API sync vs. these skills' direct MCP reads) don't silently drift into disagreeing with each other before the question resolves.

**Not yet done:** re-run the three complaint-and-event-handler evals against the fixed skill to confirm the batch-verification and wiki-context fixes hold, if Alvin wants confirmation. Also not yet done: no eval coverage exists yet for the inventory-manager / supply-demand-planner Shopify wiring above — worth a pass if Alvin wants it verified against live data the way the PD bridges were.
