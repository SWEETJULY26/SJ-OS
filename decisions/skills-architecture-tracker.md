...
- `asana-pd-manager`'s `references/stage-gate-procedure.md` documents a Formula Tracker section called "Signed Approvals" that doesn't exist anywhere in the live Asana workspace — the actual section is "Approved." Both the with-skill and baseline runs independently confirmed this via live reads. Not yet fixed — flagged here for a follow-up pass.
...
**Not yet done:** fix the asana-pd-manager "Signed Approvals" stale-doc issue found above. Re-run coverage for any of the four skills after that fix lands, if Alvin wants confirmation. All originally-shortlisted skills (the four bridges + asana-pd-manager + capa-coordinator + complaint-and-event-handler) now have real eval coverage; 51 of 58 skills in the suite remain unevaluated, out of scope unless Alvin asks to expand further.
